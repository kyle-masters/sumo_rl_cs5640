...
class SumoEnvironment(gym.Env):
    ...
    def __init__(
        ...
        if LIBSUMO:
            traci.start([sumolib.checkBinary('sumo'), '-n', self._net, '--no-step-log', str(True)])  # Start only to retrieve traffic light information
            conn = traci
        else:
            traci.start([sumolib.checkBinary('sumo'), '-n', self._net, '--no-step-log', str(True)], label='init_connection'+self.label)
        ...
        self.comp_vehicles = dict()  # Added by Kyle, tracks vehicles that have traveled through simulation
        self.vehicles = dict()
    ...
    def _get_system_info(self):
        ...
        for veh in self.vehicles:
            if veh not in vehicles and veh not in self.comp_vehicles:
                self.comp_vehicles[veh] = self.sim_step
        return {
            ...
            'system_total_completed_vehicles': len(self.comp_vehicles),
            'system_n_vehicles': len(vehicles)
        }
